...
int64_t variant::as_int() const
{
	...
	ASSERT_LOG(false, "as_int() type conversion error from {} to int", type_as_string());
	return 0;
}
...
std::string variant::as_string() const
{
	...
	ASSERT_LOG(false, "as_string() type conversion error from {} to string", type_as_string());
	return "";
}
...
float variant::as_float() const
{
	...
	ASSERT_LOG(false, "as_float() type conversion error from {} to float", type_as_string());
	return 0;
}
...
bool variant::as_bool() const
{
	...
	ASSERT_LOG(false, "as_bool() type conversion error from {} to boolean", type_as_string());
	return 0;
}
...
{
	ASSERT_LOG(type() == VARIANT_TYPE_LIST, "as_list() type conversion error from {} to list", type_as_string());
	return l_;
}

const variant_map& variant::as_map() const
{
	ASSERT_LOG(type() == VARIANT_TYPE_MAP, "as_map() type conversion error from {} to map", type_as_string());
	return m_;
}

variant_list& variant::as_mutable_list()
{
	ASSERT_LOG(type() == VARIANT_TYPE_LIST, "as_mutable_list() type conversion error from {} to list", type_as_string());
	return l_;
}

variant_map& variant::as_mutable_map()
{
	ASSERT_LOG(type() == VARIANT_TYPE_MAP, "as_mutable_map() type conversion error from {} to map", type_as_string());
	return m_;
}
...
{
	if(type() == VARIANT_TYPE_LIST) {
		return v.as_int() < static_cast<int>(l_.size()) ? true : false;
	} else if(type() == VARIANT_TYPE_MAP) {
...
glm::vec3 variant_to_vec3(const variant& v)
{
	...
	ASSERT_LOG(v.is_list() && v.num_elements() > 0 && v.num_elements() <= 3, "Expected vec3 variant but found {}", v.to_debug_string());
...
{
	ASSERT_LOG(v.is_list() && v.num_elements() == 3, "Expected ivec3 variant but found {}", v.to_debug_string());
...
{
	ASSERT_LOG(v.is_list() && v.num_elements() == 4, "Expected vec4 variant but found {}", v.to_debug_string());
	glm::quat result;
